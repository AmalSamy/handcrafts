...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Common questions Trashed</li>
@endsection
    ...
    <div class="row mx-4 mb ">
        <div class="col-lg-12 margin-tb">
            <div class="pull-right">
                <a class="btn btn-success " href="{{ route('dashboard.common_questions.index') }}"> <i
                ...
                @if ($common_questions->count())
                    @foreach ($common_questions as $common_question)
                        <tr>
                            ...
                            <td>
                                <div class="btn-group">
                                    <form action="{{ route('dashboard.common_questions.restore', $common_question->id) }}" method="post">
                                        @csrf
                                        @method('put')
                                        <button type="submit" class="btn btn-info mx-2"><strong>Restore</strong></button>
                                    </form>

                                    <form action="{{ route('dashboard.common_questions.force-delete', $common_question->id) }}" method="post">
                                        @csrf
                                        @method('delete')
